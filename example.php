<?php
/**
 * Contoh Script Bridge API Idrama via StreamAPI.web.id
 * ----------------------------------------------------
...
// Fungsi bantuan buat manggil endpoint Idrama di sistem Bridge
function callIdramaAPI($urlTujuan, $token) {
...
// UJI COBA KODE
// -----------------------------------------------------------------
$baseUrl = "https://streamapi.web.id/p/idrama/api/v1";

// Contoh 1: Fitur Cari Film Idrama
echo "\n=====================[ 1. Pencarian Idrama ]==================\n";
$kataKunci = urlencode("cinta");
$halaman = 1;
$urlCari = "{$baseUrl}/search/{$kataKunci}/{$halaman}?lang=in";

$hasilCari = callIdramaAPI($urlCari, $API_TOKEN);
if ($hasilCari) {
...
$hasilDetail = callIdramaAPI($urlDetail, $API_TOKEN);
if ($hasilDetail) {
...
$hasilNonton = callIdramaAPI($urlNonton, $API_TOKEN);
if ($hasilNonton) {
...
echo "▶▶▶ https://streamapi.web.id/docs/idrama ◀◀◀\n\n";
